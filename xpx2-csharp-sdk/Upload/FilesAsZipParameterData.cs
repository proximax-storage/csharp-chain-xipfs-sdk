...
namespace IO.Proximax.SDK.Upload
{
    public class FilesAsZipParameterData : IByteStreamParameterData
    {
        ...
        public FilesAsZipParameterData(IEnumerable<string> files, string description, string name, IDictionary<string, string> metadata)
            : base(description, name, "application/zip", metadata)
        {
            CheckParameter(files?.Any() == true, "files cannot be null or empty");
            CheckParameter(() => files.All(file => !File.GetAttributes(file).HasFlag(FileAttributes.Directory)), "not all files are file");
        ...
        public static FilesAsZipParameterData Create(IEnumerable<string> files, string description = null, string name = null, IDictionary<string, string> metadata = null) {
